	...
	public static void main (String[] args) throws Exception {
		...
		for (int position = 0; position < tokens.getLength (); position++) {
			out.format ("%s-%d ", dataAlphabet.lookupObject (tokens
					.getIndexAtPosition (position)), topics.getIndexAtPosition (position));
		}
		...
		for (int topic = 0; topic < numTopics; topic++) {
			...
			while (iterator.hasNext () && rank < 5) {
				...
				out.format ("%s (%.0f) ", dataAlphabet
						.lookupObject (idCountPair.getID ()), idCountPair.getWeight ());
				rank++;
			}
			System.out.println (out);
		}
		// Create a new instance with high probability of topic 0
		String topicZeroText = "Fascinating Early Posts From Tech Founders Who Changed The World Larry Page asked a Java question when developing Google. Just like us.";
